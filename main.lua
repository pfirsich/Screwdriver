...
function love.draw()
	...
	lg.setColor(components["Core"].static.backgroundColor)
	...
	camera.push()
		...
		if components["Core"].static.showEntityBorders then
			lg.setLineWidth(3.0/camera.scale)
			for _, entity in ipairs(map.entities) do
				if entity.__pickableComponent then
					for _, shape in ipairs(entity.__shapes) do
						...
						for i = 1, #shape, 2 do
							--lg.circle("fill", shape[i], shape[i+1], 10.0/camera.scale, 12)
						end
					end
				end
			end
		...
		lg.setLineWidth(5.0/camera.scale)
		...
		if components["Core"].static.showNames then
			for _, entity in ipairs(map.entities) do
				if entity.__pickableComponent then -- TODO: check for __shapes. Check is left out, so the program actually crashes if there are no shapes at this point (because it shouldn't happen)
					...
					local width, height = lg.getFont():getWidth(name) * nameScale, lg.getFont():getHeight() * nameScale
					...
					lg.print(name, x + shadowOffset, y + shadowOffset, 0, nameScale, nameScale)
					lg.setColor(255, 255, 255, 255)
					lg.print(name, x, y, 0, nameScale, nameScale)
				end
			end
		end
	camera.pop()
	...
	local textW, textH = lg.getFont():getWidth(modeDescription), lg.getFont():getHeight()
	local shadowOffset = 2
	lg.setColor(0, 0, 0, 255)
	lg.print(modeDescription, (winW - textW)/2 + shadowOffset, 20 + shadowOffset)
	lg.setColor(255, 255, 255, 255)
	lg.print(modeDescription, (winW - textW)/2, 20)

	if components["Core"].static.showMouseWorldPosition then
		local mouseWorldPos = {camera.screenToWorld(love.mouse.getPosition())}
		local mouseWorldPosStr = tostring(mouseWorldPos[1]) .. ", " .. tostring(mouseWorldPos[2])
		local x, y = love.mouse.getPosition()
		lg.print(mouseWorldPosStr, x, y - lg.getFont():getHeight())
	end
