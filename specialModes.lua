...
function callSpecialMode(funcName, ...)
    if specialMode then 
        if specialMode[funcName] then 
            return true, specialMode[funcName](...) 
        end
    end 
    return false
end
...
function filebrowserMode.enter(loadCB, cancelCB, startPath) -- always passes paths relative to the lfs.currentdir()
    ...
    filebrowserMode.update() -- to make sure the necessary position/size calculation have been done
...
function filebrowserMode.quit()
    if filebrowserMode.cancelButton.onClicked then filebrowserMode.cancelButton:onClicked() end
    return true
end
